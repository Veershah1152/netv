...
export const Upcoming: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useUpcomingMovies(page);
    ...
    <motion.div className="page-wrapper pt-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <div className="max-w-7xl mx-auto px-8 md:px-12 py-8">
        <div className="flex items-center gap-3 mb-8">
          <FiCalendar className="w-7 h-7 text-brand-red" />
          <h1 className="text-h1 font-bold text-white">Upcoming Movies</h1>
        </div>
        ...
        {!isLoading && !isError && (
          <>
            <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {data?.results?.map((item) => <MovieCard key={item.id} item={item} mediaType="movie" />)}
            </div>
